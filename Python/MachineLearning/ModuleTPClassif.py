...
def TesterClasses(NoClasses):
    ...
    if numMini < 1:
        print(colored(
            'Attention, les numeros de classe doivent commencer à 1 (ici {0}). La valeur 0 est réservée pour désigner l''ensemble des individus (voir la fonction CalculerCentresGravite).\n'.format(
                numMini), 'red'))
...
def CalculerMatricesCovarianceInverseClasses(Individus, NoClasses, CentresGravite):
    ...
    if NbrClasses == 1:
        ...
        if not np.allclose(np.dot(MatriceCovarianceClasse, np.linalg.inv(MatriceCovarianceClasse)),
                           np.eye(NbrVariables)):
            print(colored(
                'Attention : probleme lors de l''inversion de la matrice de covariance de la classe {0:} !\n'.format(q),
                'red'))
        else:
        ...
        for q in range(1, NbrClasses):
            IndClasse = np.argwhere(NoClasses == q)[:, 0]
            MatriceCovarianceClasse = np.dot(IndividusCentresClasse[IndClasse].T,
                                             IndividusCentresClasse[IndClasse]) / LngIndClasse
...
def PresenterClasses(Individus, NoClasses, Titre, CentresGravite=[], NomParametres=[], MaxGraphes=10, ParamX=0,
    ...
    if ((ParamX != 0) or (ParamY != 0)) and (ParamX != ParamY):
        ...
        if nbrGraphes > MaxGraphes:
            print(colored(
                'Attention, le nombre de graphiques est limité à {0} : {1} paramètres pris deux à deux, donnent {2} graphiques.'.format(
                    MaxGraphes, NbrParametres, nbrGraphes), 'red'))
